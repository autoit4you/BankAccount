...
public class CommandAccountRemoveuser extends BankAccountCommand {
    ...
    public void run(CommandSender sender, String[] args, BankAccount plugin)
        ...
        if(!plugin.getPermissions().user(sender, Permissions.Permission.ACCOUNT_MANAGE_USER))
        ...
        try {
            ...
            if(account.getAccess(sender.getName()) < 2) {
                ...
                sender.sendMessage(ChatColor.GOLD + args[2] + ChatColor.GREEN + " has no longer user access to the account " + args[1] + "");
            }
